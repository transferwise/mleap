...
import com.truecar.mleap.linalg
  ...
  describe("#apply") {
    describe("with mean") {
      it("scales based off of the mean") {
        val scaler = StandardScalerModel(None, Some(linalg.Vector.dense(Array(50.0, 20.0, 30.0))))
    ...
    describe("with stdev") {
      it("scales based off the standard deviation") {
        val scaler = StandardScalerModel(Some(linalg.Vector.dense(Array(2.5, 8.0, 10.0))), None)
    ...
    describe("with mean and stdev") {
      it("scales based off the mean and standard deviation") {
        val scaler = StandardScalerModel(Some(linalg.Vector.dense(Array(2.5, 8.0, 10.0))),
