...
lazy val `mleap-core` = project.in(file("mleap-core"))
  ...
  .settings(libraryDependencies ++= Dependencies(version.value).mleapCoreDependencies)

lazy val `mleap-runtime` = project.in(file("mleap-runtime"))
  .settings(Common.settings)
  .settings(Common.sonatypeSettings)
  .settings(libraryDependencies ++= Dependencies(version.value).mleapRuntimeDependencies)
  .dependsOn(`mleap-core`)

lazy val `mleap-spark` = project.in(file("mleap-spark"))
  .settings(Common.settings)
  .settings(Common.sonatypeSettings)
  .settings(libraryDependencies ++= Dependencies(version.value).mleapSparkDependencies)
